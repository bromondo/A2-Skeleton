...
def main():
    ...
    train_loader, val_loader, test_loader, vocabs = getMulti30kDataloadersAndVocabs(config)

    print("English:" + str(vocabs["en"]))
    print("German:" + str(vocabs["de"]))
    ...
    #try:
    #  model = torch.compile(de_to_ge_model)
    #except:
...
def generateAttentionTranslationPlots(model, val_loader, vocabs, max_len=25):
  ...
  for i in range(min(8,x.shape[0])):
    translation = vocabs["en"].detokenize(out[i,:].detach().cpu().numpy().astype(int))
    ...
    source = vocabs["de"].detokenize(x[i,:].detach().cpu().numpy())
    for t, w in enumerate(source):
       if w == '<EOS>':
          source = source[:t+1]
          break
    
    attn_mat = attn_mat[:, :t+1]

    actual_translation = vocabs["en"].detokenize(y[i,:].detach().cpu().numpy())
